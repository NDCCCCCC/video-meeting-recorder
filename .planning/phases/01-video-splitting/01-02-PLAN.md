...
must_haves:
  truths:
    ...
    - "Snapshot files are registered as VideoFile records with source_type=snapshot"
...
<task type="auto">
  ...
  <behavior>
    ...
    - SnapshotService copies partial MKV to temp location, converts to MP4 via FFmpeg, registers via CreateSegmentFile with sourceType=snapshot
...
func (s *SnapshotService) GenerateSnapshot(taskID uint, createdBy uint) (*models.VideoFile, error) {
    ...
    // 4. Copy partial MKV to temp file (avoid locking issues with active recording)
    ...
    // 7. Find the parent VideoFile for this task
    ...
    // 8. Register snapshot file via VideoFileService callback (D-10, D-13)
    snapshotFile, err := s.videoFileService.CreateSegmentFile(outputMP4, &parentID, models.SourceTypeSnapshot, createdBy)
    if err != nil {

...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 24-01-01 | 01 | 1 | DETECT-02 | T-24-01 / Tampering | 严格 regex 匹配 silencedetect stderr 行；非匹配行静默丢弃 | unit (fixture matrix) | `go test ./internal/recorder -run 'TestSilenceParser' -count=1` | ❌ W0 | ⬜ pending |
| 24-01-02 | 01 | 1 | DETECT-02 | T-24-02 / — | `silence_duration >= silence_duration_s` 才触发 A 信号 | unit (fixture) | `go test ./internal/recorder -run 'TestSilenceParser_DurationThreshold' -count=1` | ❌ W0 | ⬜ pending |
| 24-02-01 | 02 | 1 | DETECT-03 | — / N/A | ticker 周期 stat + rolling window + min_growth_bps 判定 | unit (fake StatFn) | `go test ./internal/recorder -run 'TestActivityWatcher_FileStall' -count=1` | ❌ W0 | ⬜ pending |
| 24-02-02 | 02 | 1 | WATCH-04 | T-24-03 / DoS | stat 连续 `stat_failure_threshold` 次失败 → 发 `EventMeetingEnded reason=file_stat_failed` | unit | `go test ./internal/recorder -run 'TestActivityWatcher_StatFailed' -count=1` | ❌ W0 | ⬜ pending |
| 24-03-01 | 03 | 1 | WATCH-01 | — / N/A | H+A+B OR 判定 + sync.Once close-once + EndedReason 字段 | unit (fake Huawei + fake clock + fake logFile) | `go test ./internal/recorder -run 'TestActivityWatcher_MeetingEnded' -count=1` | ❌ W0 | ⬜ pending |
| 24-03-02 | 03 | 1 | WATCH-01 | — / N/A | H 持续 `huawei_persist_s` 触发；A+B 双 AND 触发 | unit | `go test ./internal/recorder -run 'TestActivityWatcher_HuaweiPersist|TestActivityWatcher_AndAB' -count=1` | ❌ W0 | ⬜ pending |
| 24-04-01 | 04 | 1 | WATCH-02 | T-24-04 / DoS | silencedetect 解析 5 次失败 → silenceDegraded=true，A 路径不再触发 | unit | `go test ./internal/recorder -run 'TestActivityWatcher_SilenceDegraded' -count=1` | ❌ W0 | ⬜ pending |
| 24-04-02 | 04 | 1 | WATCH-03 | T-24-04 / DoS | Huawei 连续 `huawei_failure_threshold` 次失败 → huaweiDegraded=true，H 路径不再触发 | unit | `go test ./internal/recorder -run 'TestActivityWatcher_HuaweiDegraded' -count=1` | ❌ W0 | ⬜ pending |
| 24-05-01 | 05 | 1 | WATCH-05 | T-24-05 / EoP | `OnReconnect` 回调清 `SilenceSince` 不动文件 ticker / H 状态 | unit | `go test ./internal/recorder -run 'TestActivityWatcher_Reconnect' -count=1` | ❌ W0 | ⬜ pending |
| 24-05-02 | 05 | 1 | WATCH-05 | T-24-05 / — | `restartRecording` 同步调用 `OnReconnect()`（如已注册） | unit (coordinator extension) | `go test ./internal/recorder -run 'TestRestartRecording_OnReconnect' -count=1` | ❌ W0 (扩展现有) | ⬜ pending |
| 24-06-01 | 06 | 1 | EXTEND-03 | — / N/A | `ActivityWatcher.ExtendStepMin()` 返回 cfg 值（默认 30min） | unit | `go test ./internal/recorder -run 'TestActivityWatcher_ExtendStep' -count=1` | ❌ W0 | ⬜ pending |
| 24-06-02 | 06 | 1 | DETECT-02 (wiring) | T-24-01 / — | `buildRecordingCommand` 包含 `-af silencedetect=noise=...:d=...` | unit (coordinator extension) | `go test ./internal/recorder -run 'TestBuildRecordingCommand_SilenceDetect' -count=1` | ❌ W0 (扩展现有) | ⬜ pending |
| 24-06-03 | 06 | 1 | CFG-01 (依赖) | — / — | `cfg.SmartEnd.*` 字段在 `ExtendStepMin()` 调用时正确读取（Phase 23 CFG-01 验收通过即可） | unit (config integration) | `go test ./internal/config -run 'TestSmartEndConfig' -count=1` | ✅ (Phase 23) | ⬜ pending |
...
- [ ] No watch-mode flags
- [ ] Feedback latency < 60s（quick run ~10s）

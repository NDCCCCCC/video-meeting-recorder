...
func (c *SimpleRecordingCoordinator) startFFmpegProcess(cmd *exec.Cmd, outputPath, configType string) (*os.File, error) {
	// Bug F: 给每个 ffmpeg 进程独立 log 文件,避免 Windows 文件锁截断。
	// 多个 ffmpeg 共享同一 ffmpeg.log 时,后启动的 os.Create 会截断前一个
	// 持有的文件,导致前一个 ffmpeg 的 stderr 写入被静默丢弃。
	logFileName := "ffmpeg_" + configType + ".log"
	logPath := filepath.Join(filepath.Dir(outputPath), logFileName)
